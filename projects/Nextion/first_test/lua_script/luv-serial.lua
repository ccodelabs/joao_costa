...
-- function to read a text line with timeout in msec
-- if no length is given, 64 is assumed
-- function returns a string

function SerialPort:readln_timeout(timeout, len)
    len = len or 64
    local this = self
    local cr = coroutine.running()
    data = nil
    ...
    self._poll:start('rd', function(_, events)
        err, d, size = this._port:read(1)

        if (d and d:len()>0) then
          if (d:byte(1)>=32) then 		-- is printable character
            if data then data = data .. d else data=d end
          else
            this._poll:stop()
            timer:close()
            coroutine.resume(cr)
