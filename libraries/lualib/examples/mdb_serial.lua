...
function module.initializeMdbDevice(on_connect, on_disconnect, on_data_read)
  --Get serial handle
  for i=0,10,1 do
    local p = Serial{device="/dev/ttyAMA0", baudrate=115200, databits=8, parity="none", stopbits=1, xonxoff=false, rtscts=false};
  	if p~=nil then
  	  module.p = p;
      break;
    end
