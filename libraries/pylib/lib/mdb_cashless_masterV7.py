...
def end_transaction(product,ser):
    print('Waiting for transaction to be confirmed...')
    res=read_and_wait(ser)
    if res.find('d,STATUS,RESULT,1')!=-1:
        money = res.split(',')[4]
        print('Success! Deviced was charged for '+money+', for product '+product)
...
if  ser.is_open:
    ...
    user_inp=input("Enter the amount and the product to dispense, separated by a space and hit enter\n(ex.:1.2 10): ").strip()
    while user_inp=='' or not user_inp.replace(" ","").isdigit() or user_inp.find(' ')==-1: #wait for valid user input
        user_inp=input("Bad arguments! Please try again...").strip()
    amount,product=user_inp.split()
